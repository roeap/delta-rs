...
- **D3 — expose the opaque predicate adaptor.**
  ...
  ArrowOpaquePredicateOp` accessor. Required for delta-rs to downcast-recover its
  `DataFusionOpaquePredicateOp` from a `Predicate::Opaque` (the
  `predicate_to_df` round-trip). Purely additive; upstream candidate (kernel
  TODO #1564 tracks opaque serde gaps). Without it the D3 round-trip does not
  compile.
