...
#[cfg(feature = "tracing")]
use crate::delta_datafusion::session::INSTRUMENT_RULE;
...
impl ParquetHandler for DataFusionFileFormatHandler {
    #[instrument(skip(self, physical_schema), level = "info")]
    fn read_parquet_files(
        ...
        #[cfg(feature = "tracing")]
        let exec = INSTRUMENT_RULE
            .optimize(exec, self.ctx.session_config().options().as_ref())
            .map_err(Error::generic_err)?;
    ...
    #[instrument(skip(self, _data), level = "info")]
...
fn parquet_exec(
    ...
    let mut file_source = ParquetSource::new(read_schema.clone())
...
fn json_exec(
    ...
    let config = FileScanConfigBuilder::new(store_url, Arc::new(JsonSource::new(arrow_schema)))
