...
/// A Tokio runtime [`Handle`] that propagates the current `tracing` span across the
/// async/sync boundary.
///
...
impl DataFusionEngine {
    ...
    /// engine to a specific runtime handle rather than the ambient one. `handle` accepts a
    /// [`Handle`] or a [`TracedHandle`]; D2 swaps this for a target-portable executor.
...
pub(crate) struct BlockingStreamIterator<T: Send + 'static> {
    pub(crate) stream: Option<BoxStream<'static, T>>,
    pub(crate) handle: TracedHandle,
}
